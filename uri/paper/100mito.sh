...
start=$(date +%s.%N)
python -m fastai.launch train.py --bs 24 --lr 5e-4 --size 256 --tile_sz 512 --datasetname single_100mitotracker --cycles 40 --save_name single_100mito --lr_type s --n_frames 1
dur=$(echo "$(date +%s.%N) - $start" | bc) >> 100mito_trainlog.txt
printf "single_100mito_round1 - Execution time: %.6f seconds" $dur >> 100mito_log.txt

start=$(date +%s.%N)
python -m fastai.launch train.py --bs 8 --lr 1e-4 --size 512 --tile_sz 512 --datasetname single_100mitotracker --cycles 30 --save_name single_100mito --lr_type s --n_frames 1 --load_name single_100mito_best_256 --freeze >> 100mito_trainlog.txt
dur=$(echo "$(date +%s.%N) - $start" | bc)
printf "single_100mito_round2 - Execution time: %.6f seconds" $dur >> 100mito_log.txt

start=$(date +%s.%N)
python -m fastai.launch train.py --bs 8 --lr 1e-4 --lr_start 1e-6 --size 512 --tile_sz 512 --datasetname single_100mitotracker --cycles 40 --save_name single_100mito --lr_type s --n_frames 1 --load_name single_100mito_best_512 >> 100mito_trainlog.txt
dur=$(echo "$(date +%s.%N) - $start" | bc)
...
start=$(date +%s.%N)
python -m fastai.launch  train.py --bs 24 --lr 5e-4 --size 256 --tile_sz 512 --datasetname multi_100mitotracker --cycles 40 --save_name multit_100mito --lr_type t --n_frames 5
dur=$(echo "$(date +%s.%N) - $start" | bc) >> 100mito_trainlog.txt
printf "multi_100mito_round1 - Execution time: %.6f seconds" $dur >> 100mito_log.txt

start=$(date +%s.%N)
python -m fastai.launch  train.py --bs 8 --lr 1e-4 --size 512 --tile_sz 512 --datasetname multi_100mitotracker --cycles 30 --save_name multit_100mito --lr_type t --n_frames 5 --load_name multit_100mito_best_256 --freeze >> 100mito_trainlog.txt
dur=$(echo "$(date +%s.%N) - $start" | bc)
printf "multi_100mito_round2 - Execution time: %.6f seconds" $dur >> 100mito_log.txt

start=$(date +%s.%N)
python -m fastai.launch  train.py --bs 8 --lr 1e-4 --lr_start 1e-6 --size 512 --tile_sz 512 --datasetname multi_100mitotracker --cycles 40 --save_name multit_100mito --lr_type t --n_frames 5 --load_name multit_100mito_best_512 >> 100mito_trainlog.txt
dur=$(echo "$(date +%s.%N) - $start" | bc)
