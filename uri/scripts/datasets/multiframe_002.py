...
def crappify_range(img_data, scale=4):
    lrs_down = []
    lrs_up = []
    for i in range(img_data.shape[0]):
        ...
        lrs_up.append(lr_up)
    lr_imgs = np.array(lrs_down).astype(np.float32)
    lr_up_imgs = np.array(lrs_up).astype(np.float32)
...
for lst in [hr_airyscan, one_channel, two_channel]:
    ...
    for subdir, file_list in [('train', train_files),('valid', valid_files)]:
        ...
        for czi_fn in pbar:
            czi_to_multiframe(czi_fn, hr_path/subdir, lr_path/subdir, lr_up_path/subdir, wsize=5, pbar=pbar)
